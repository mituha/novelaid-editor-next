...
前回、ファイル一覧部分のアイコン表示を調整しました。  
今回もファイル一覧部分ですが、少しElectronとTauriの仕組みを見ていきます。
...
ちなみに、この図はAIに[mermaid.js]()https://mermaid.js.org/)で描画するように指示して作成しました。
...
記事中に用いたこの図ですが、実は以下のようにテキスト形式で記述しています。  
...
graph TD
    ...
    %% 関係性
    Grandpa -- "育てる" --- Momotaro
    Grandma -- "育てる" --- Momotaro
    
    Momotaro -- "きびだんごを与える" --- Dog
    Momotaro -- "きびだんごを与える" --- Monkey
    Momotaro -- "きびだんごを与える" --- Pheasant
    
    Dog -- "お供/共闘" --- Momotaro
    Monkey -- "お供/共闘" --- Momotaro
    Pheasant -- "お供/共闘" --- Momotaro

    Momotaro -- "退治/敵対" --- Oni
    Oni --- OniSub
    
    %% スタイル設定
    style Momotaro fill:#ffcccc,stroke:#ff6666,stroke-width:2px
    style Oni fill:#990000,stroke:#330000,color:#fff
```
こんな感じで人物相関図っぽいのも描けます。  
マークダウン上で済むため、修正も楽です。  
ちなみに、この図も「桃太郎における人物相関図をmermaid.jsの書式で書いて」とAI君に頼んで生成したものです。
ただ、「[]内等、""で括る必要があります」と追加の指示が必要でした。

で、この機能はまだ自作の小説執筆アプリには追加されていません。  
マークダウンのプレビュー表示までは実装されていますので、追加する際は記事にすると思います。
